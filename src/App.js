...
class App extends React.Component {
  ...
  handleQuery = (event) => {
    ...
    const queriedFood = foods.filter((food) =>
  ...
  removeFoodToday(name) {
    const foodTodayCopy = JSON.parse(JSON.stringify(this.state.foodToday));
    const subCal = foodTodayCopy[name].totalCal;
    delete foodTodayCopy[name];
    this.setState({
      foodToday: foodTodayCopy,
      totalCal: this.state.totalCal - subCal
    })
  }
  ...
  render() {
    const foodList = this.state.displayedFoods.map((food, index) => {
      return (
        <FoodBox key={index} food={food} addFoodToday={this.addFoodToday} />
      );
    });
    ...
    const todayFoods = Object.keys(this.state.foodToday).map((key, index) => {
      return (
        <li className="has-text-left" key={index}>
          {this.state.foodToday[key].quantity} {key} ={' '}
          {this.state.foodToday[key].totalCal} cal{' '}
          <i
            className="fas fa-trash fa-lg has-text-danger"
            onClick={() => this.removeFoodToday(key)}
          ></i>
        </li>
      );
    });
    return (
      <div className="App">
        ...
        <input
          ...
          type="text"
          placeholder="Searching for a food"
        ...
        <div className="columns">
          ...
          <div className="m-3 pl-5 column">
            ...
            <ul>{todayFoods}</ul>
            <hr />
            <h3 className="is-size-5 has-text-left">
              <i>Total:</i> {this.state.totalCal} cal{' '}
            </h3>
          </div>
        </div>
      </div>
