...
function TodaysList(props) {
  ...
  const foodsArray = props.list.map((item) => {
    if (item.quantity !== 0) {
      return (
        <li key={props.list.indexOf(item)} className="has-text-justified">
          {item.quantity} {item.name} = {item.quantity * item.calories}
        </li>
      );
    } else {
      return console.log('Nothing in today`s list');
    }
  });
  return (
    <div>
