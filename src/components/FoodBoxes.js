...
function FoodBoxes(props) {
  ...
  const handleChange = (event) => {
    var index = todaysList.items.findIndex((item) =>
  ...
  const printInfo = (event) => {
    ...
    var indexList = newList.findIndex((item) =>
      item.name.includes(event.target.name)
    );
    if (indexList !== -1) {
      newList[indexList] = {
        ...newList[indexList],
        quantity: todaysList.items[index].quantity,
      };
    } else {
  ...
  const foodsArray = foods.map((item) => (
    <FoodBox
      key={props.foods.indexOf(item)}
      name={item.name}
  ...
  if (todaysList.list.length !== 0) {
    return (
      <div className="columns">
        <div className="column is-half">{foodsArray}</div>
